...
qqplot(data_G0$Nb_adults, x.teo.negbin, main="QQ-plot", xlab="Observed data", ylab="Simulated data", las=1) ## QQ-plot
...
colnames(overdisp) <- c("fruit", "generation", "fitted_mean_nb_adults", "fitted_theta", "fitted_var_nb_adults", "obsoverdisp", "fittedoverdisp", "standardized_mean", "standardized_mean_small_smaple")
...
## Combine datasets
overdisp <- rbind(overdispG0, overdispG7, overdispG29)
colnames(overdisp) <- c("factor", "generation", "number_tubes_counted", "fitted_mean_nb_adults", "fitted_theta", "observed_var_nb_adults", "fitted_var_nb_adults", "obsoverdisp", "fittedoverdisp", "standardized_mean", "standardized_mean_small_smaple")
rownames(overdisp) <- 1:nrow(overdisp)
...
## Only population CE2 and CR2 do not pass the threshold of a standardized mean greater than 3
```
...
hist(sim$boot_varestimfitness, xlab="Variance estimate based on boostrap")
...
for (ntubes_ancestor in Ntube){
  ...
  bias <- c(bias, mean(sim$expected_meanfitness[!is.infinite(sim$bootstrapped_meanfitness)]-sim$bootstrapped_meanfitness[!is.infinite(sim$bootstrapped_meanfitness)]))
  power <- c(power, mean(sim$indic))
}

## Small bias
bias

plot(Ntube, power, xlab="Number of tubes", ylab="Power", ylim=c(0, 1), las=1, type="l", xaxt='n')
...
for (ntubes_tested in Ntube){
  sim <- data.frame(t(sapply(1:500, computemeanvar_fitnessdifferencedata, ntubes_ancestor=30, N_ancestor=25, theta_ancestor=3, ntubes_derived=ntubes_tested, N_derived=30, theta_derived=3)))
  bias <- c(bias, mean(sim$expected_meanfitnessdiff[!is.infinite(sim$bootstrapped_meanfitnessdiff)]-sim$bootstrapped_meanfitnessdiff[!is.infinite(sim$bootstrapped_meanfitnessdiff)]))
  power <- c(power, mean(sim$indic))
}
