...
sim <- data.frame(t(sapply(1:1000, computemeanvar_fitnessdifferencedata, ntubes_ancestor=100, N_ancestor=25, theta_ancestor=3, ntubes_derived=30, N_derived=30, theta_derived=3)))
...
for (ntubes_tested in Ntube){
  sim <- data.frame(t(sapply(1:10000, computemeanvar_fitnessdifferencedata, ntubes_ancestor=100, N_ancestor=25, theta_ancestor=3, ntubes_derived=ntubes_tested, N_derived=30, theta_derived=3)))
...
## Check coverage
data.frame(Ntube, coveragenormapprox)

pdf(file="figures/FIG_SX_confidenceinterval_Nb_adults.pdf")

plot(Ntube, coveragenormapprox, xlab="Number of vials", ylab="Coverage of the confidence interval", xlim=c(0, 30), ylim=c(0, 1), las=1, type="l", xaxt='n', lty=1, bty='n')
...
# Check delta method approximation for the number of eggs
## Compare estimates per experimental population
```{r }
...
## Check standardized mean
overdisp[as.numeric(as.character(overdisp$standardized_mean_small_sample))<3, ]
## All population pass Geary's test (the threshold of a standardized mean greater than 3)
