...
estim_overdisp <- function(colfactorlevel, colfactor, data, generation){
  ...
  ## Get parameters
  param <- c(length(data$Nb_adults), as.numeric(exp(coef(m))), m$theta, var(data$Nb_adults), as.numeric(exp(coef(m))+(exp(coef(m))^2)/m$theta), var(data$Nb_adults)/mean(data$Nb_adults), 1+as.numeric(exp(coef(m)))/m$theta, ((4*length(data$Nb_adults)^(3/2))/(1+4*length(data$Nb_adults)))* (mean(data$Nb_adults)/sd(data$Nb_adults)), sqrt(length(data$Nb_adults))* (mean(data$Nb_adults)/sd(data$Nb_adults)))
